...
struct implication : public dense_timed_node<robustness_interval_map<T, V>, T> {
  using time_t = T;
  using input_t = X;
  using value_t = V;
  ...
  output_t output(time_t previous, time_t now) {
    auto negated_arg1 = interval_map();
    for (const auto &intv : arg1->output(previous, now)) {
      negated_arg1.add(std::make_pair(intv.first, -intv.second));
    }
    return negated_arg1 + arg2->output(previous, now);
  }
};
