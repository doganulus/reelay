...
struct since_bounded_half : public dense_timed_state<X, interval_set<T>, T>
{
    using time_t = T;
    using input_t = X;
    ...
    since_bounded_half(std::vector<node_ptr_t> args, time_t l) : first(args[0]), second(args[1]), lbound(l) {}
    ...
    void update(bool p1, bool p2, time_t previous, time_t now)
    {
        ...
        if(p1 and p2){
            ...
            value.add(interval::left_open(previous + lbound, std::numeric_limits<time_t>::max()));
            ...
            value.add(interval::left_open(now + lbound, std::numeric_limits<time_t>::max()));
    ...
    void update(const input_t &pargs, const input_t &args, time_t previous, time_t now) override
    {
         ...
         *  calls the _update function for each constant period sequentially. The synchronization 
        ...
        value = value & interval_set(interval::left_open(previous, std::numeric_limits<time_t>::max()));
