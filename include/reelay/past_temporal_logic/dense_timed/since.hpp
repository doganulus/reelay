...
struct since : public dense_timed_state<X, interval_set<T>, T>
{
    using time_t = T;
    using input_t = X;
    ...
    void update(bool p1, bool p2, time_t previous, time_t now)
    {
        ...
        if(p1 and p2){
            value.add(interval::left_open(previous, std::numeric_limits<time_t>::max()));
            ...
            // Comment out the line below for lazy trimming
            // state = state & interval_set(interval::left_open(previous, std::numeric_limits<time_t>::max()));
        } else {
            value = value & interval::left_open(0, previous);
        }
    }
