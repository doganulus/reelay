...
struct factory
{
    ...
    static state_ptr_t make_proposition(const std::string &name){
        return std::make_shared<discrete_timed_setting::proposition<input_t,time_t>>(name);
    }
    ...
    static state_ptr_t make_basic_predicate(
        ...
        if (op == "lt" or op == "<") {
            result = std::make_shared<discrete_timed_setting::basic_predicate_lt<input_t,time_t>>(name,c);
        } else if (op == "le" or op == "leq" or op == "<=") {
            result = std::make_shared<discrete_timed_setting::basic_predicate_le<input_t,time_t>>(name,c);
        } else if (op == "gt" or op == ">") {
            result = std::make_shared<discrete_timed_setting::basic_predicate_gt<input_t,time_t>>(name,c);
        } else if (op == "ge" or op == "geq" or op == ">=") {
            result = std::make_shared<discrete_timed_setting::basic_predicate_ge<input_t,time_t>>(name,c);
        } else {
            throw std::invalid_argument("Unsupported predicate operator for the discrete timed setting");
        }
    ...
    static state_ptr_t make_predicate(const function_t &fn){
        return std::make_shared<discrete_timed_setting::predicate<input_t,time_t>>(fn);
    }
    ...
    static node_ptr_t make_node(
        ...
        if (name == "disjunction"){
            result = std::make_shared<discrete_timed_setting::disjunction<input_t,time_t>>(args);
        } else if (name == "conjunction"){
            result = std::make_shared<discrete_timed_setting::conjunction<input_t,time_t>>(args);
        } else if (name == "negation"){
            result = std::make_shared<discrete_timed_setting::negation<input_t,time_t>>(args);
        } else if (name == "implication") {
            result = std::make_shared<discrete_timed_setting::implication<input_t,time_t>>(args);
        } else if (name == "past_sometime") {
            result = std::make_shared<discrete_timed_setting::past_sometime<input_t,time_t>>(args);
        } else if (name == "past_always") {
            result = std::make_shared<discrete_timed_setting::past_always<input_t,time_t>>(args);
        } else if (name == "since") {
            result = std::make_shared<discrete_timed_setting::since<input_t,time_t>>(args);
        } else {
            throw std::invalid_argument("Unsupported operator for the discrete timed setting");
        }
    ...
    static state_ptr_t make_state(
        ...
        if (name == "past_sometime") {
            ...
            if(u > 0){
                result = std::make_shared<discrete_timed_setting::past_sometime_bounded<input_t,time_t>>(args,l,u);
            } else {
                result = std::make_shared<discrete_timed_setting::past_sometime_bounded_half<input_t,time_t>>(args,l);
            }
            ...
            if(u > 0){
                result = std::make_shared<discrete_timed_setting::past_always_bounded<input_t,time_t>>(args,l,u);
            } else {
                result = std::make_shared<discrete_timed_setting::past_always_bounded_half<input_t,time_t>>(args,l);
            }
            ...
            if(u > 0){
                result = std::make_shared<discrete_timed_setting::since_bounded<input_t,time_t>>(args,l,u);
            } else {
                result = std::make_shared<discrete_timed_setting::since_bounded_half<input_t,time_t>>(args,l);
            }
