...
template <typename X> struct exists : public untimed_node<data_set_t> {
  using input_t = X;
  ...
  explicit exists(const data_mgr_t &mgr, const std::vector<std::string> &vars,
      ...
      explicit exists(const kwargs &kw)
          : exists(std::any_cast<data_mgr_t>(kw.at("manager")),
