...
struct factory {
    ...
    static state_ptr_t make_basic_predicate(
        ...
        if (op == "lt" or op == "<") {
            result = std::make_shared<untimed_setting::basic_predicate_lt<input_t>>(name,c);
        } else if (op == "le" or op == "leq" or op == "<=") {
            result = std::make_shared<untimed_setting::basic_predicate_le<input_t>>(name,c);
        } else if (op == "gt" or op == ">") {
            result = std::make_shared<untimed_setting::basic_predicate_gt<input_t>>(name,c);
        } else if (op == "ge" or op == "geq" or op == ">=") {
            result = std::make_shared<untimed_setting::basic_predicate_ge<input_t>>(name,c);
        } else {
    ...
    static node_ptr_t make_node(
        ...
        if (name == "disjunction"){
            ...
            throw std::invalid_argument("Unsupported operator for the untimed setting");
        }
