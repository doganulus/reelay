...
struct dense_timed {
  ...
  struct robustness {
    ...
    struct monitor {
      ...
      options()
      {
        return dense_robustness_monitor_options<
          TimeT,
          ValueT,
          InputT,
          OutputT>();
...
struct discrete_timed {
  ...
  struct robustness {
    ...
    struct monitor {
      ...
      options()
      {
        return discrete_robustness_monitor_options<
          TimeT,
          ValueT,
          InputT,
          OutputT>();
  ...
  const dense_monitor_options<TimeT, InputT, OutputT>& options)
{
  ...
  if(reelay::any_cast<bool>(inspection["has_references"])) {
    return type(
      dense_timed_data_monitor<time_type, input_type, output_type>::make_shared(
  ...
  const discrete_monitor_options<TimeT, InputT, OutputT>& options)
{
  ...
  if(
    reelay::any_cast<bool>(inspection["has_references"]) and
    options.is_condensing()) {
    return type(
  ...
  if(
    not reelay::any_cast<bool>(inspection["has_references"]) and
    options.is_condensing()) {
    return type(
    ...
    options)
{
  ...
  if(
    not reelay::any_cast<bool>(inspection["has_references"]) and
    options.is_condensing()) {
    return type(discrete_timed_robustness_monitor<
                time_type,
                value_type,
                input_type,
                output_type,
                true>::make_shared(pattern, options.get_basic_options()));
  }
  ...
  if(
    ...
    return type(discrete_timed_robustness_monitor<
                ...
                input_type,
                output_type,
                false>::make_shared(pattern, options.get_basic_options()));
  }
