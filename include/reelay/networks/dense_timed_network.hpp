...
struct dense_timed_network : dense_timed_state<input_t, output_t, time_t>
{
    ...
    void update(const input_t &pargs, const input_t &args, time_t previous, time_t now) noexcept override
    {
    ...
    std::string output_verbosity_0()
    {
        ...
        for(const auto& intv : result){
            buffer << "Fails between times " << intv.lower() << " and " << intv.upper() << std::endl;    
        }
    ...
    std::string output_verbosity_1()
    {
        ...
        if (result.empty())
        {
            buffer << this->current << "," << "0" << std::endl;
        }

        for(const auto& intv : result){
            if(intv.lower() > this->previous){
                t = intv.upper();
                buffer << intv.lower() << "," << "0" << std::endl;
                buffer << intv.upper() << "," << "1" << std::endl;
