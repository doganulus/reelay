...
struct discrete_timed_json_monitor : base_monitor<TimeT, json, json> {
  using time_t = TimeT;
  using input_t = json;
  using output_t = json;
  ...
  discrete_timed_json_monitor() = default;

  explicit discrete_timed_json_monitor(const std::string &pattern,
