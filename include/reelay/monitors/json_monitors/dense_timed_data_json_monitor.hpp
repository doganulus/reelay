...
#include "reelay/targets/json/dense_timed_data_json_formatter.hpp"

namespace reelay {

template <
    typename TimeT, typename InputT = reelay::json,
    typename OutputT = std::vector<InputT>,
    class FormatterT = dense_timed_data_json_formatter<TimeT, bool, OutputT>>
struct dense_timed_data_json_monitor final
  ...
  dense_timed_data_json_monitor() = default;

  explicit dense_timed_data_json_monitor(const std::string &pattern,
  ...
  output_t update(const input_t &args) {
    ...
    return formatter.format(manager, network->output(), network->previous,
                            network->current);
  }
