...
struct condensing<T>::json_monitor {
  ...
  boost::variant<untimed_condensing_json_monitor<time_t>,
                 untimed_data_condensing_json_monitor<time_t>,
                 discrete_timed_condensing_json_monitor<time_t>,
                 discrete_timed_data_condensing_json_monitor<time_t>>
  ...
  explicit json_monitor(const std::string &pattern,
    ...
    if (not timed and not categorical) {
      _impl = untimed_condensing_json_monitor<time_t>(pattern, kw);
    } else if (timed and not categorical) {
      _impl = discrete_timed_condensing_json_monitor<time_t>(pattern, kw);
    } else if (not timed and categorical) {
      _impl = untimed_data_condensing_json_monitor<time_t>(pattern, kw);
    } else {
      _impl = discrete_timed_data_condensing_json_monitor<time_t>(pattern, kw);
    }
  }

  struct visitor_update : boost::static_visitor<output_t> {
    output_t operator()(untimed_condensing_json_monitor<time_t> &monitor,
                        const input_t &object) const {
      return monitor.update(object);
    }
    output_t operator()(untimed_data_condensing_json_monitor<time_t> &monitor,
                        const input_t &object) const {
      return monitor.update(object);
    }
    output_t operator()(discrete_timed_condensing_json_monitor<time_t> &monitor,
                        const input_t &object) const {
      return monitor.update(object);
    }
    output_t operator()(
        discrete_timed_data_condensing_json_monitor<time_t> &monitor,
        const input_t &object) const {
      return monitor.update(object);
    }
  };
  ...
  struct visitor_now : boost::static_visitor<time_t> {
    time_t operator()(untimed_condensing_json_monitor<time_t> &monitor) const {
      return monitor.now();
    }
    time_t operator()(
        untimed_data_condensing_json_monitor<time_t> &monitor) const {
      return monitor.now();
    }
    time_t operator()(
        discrete_timed_condensing_json_monitor<time_t> &monitor) const {
      return monitor.now();
    }
    time_t operator()(
        discrete_timed_data_condensing_json_monitor<time_t> &monitor) const {
      return monitor.now();
    }
  };
