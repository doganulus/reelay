...
struct discrete_timed_monitor final : public abstract_monitor<InputT, OutputT> {
  using time_type = TimeT;
  using value_type = bool;
  using input_type = InputT;
  using output_type = OutputT;

  using type = discrete_timed_monitor<
      TimeT, InputT, OutputT, condensing>;

  using node_type = discrete_timed_node<value_type, time_type>;
  using state_type
      = discrete_timed_state<input_type, value_type, time_type>;

  using network_type = discrete_timed_network<time_type, input_type>;

  using network_t = discrete_timed_network<time_type, input_type>;
  using formatter_t
      = discrete_timed_formatter<time_type, value_type, output_type, condensing>;

  discrete_timed_monitor() = default;

  explicit discrete_timed_monitor(const network_t &n, const formatter_t &f)
