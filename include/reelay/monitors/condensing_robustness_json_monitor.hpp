...
struct condensing<T>::robustness<V>::json_monitor {
  ...
  static base_ptr_t make(const std::string &pattern,
    ...
    if (not timed) {
      return std::make_shared<untimed_robustness_condensing_json_monitor<time_t, value_t>>(pattern,
                                                                        kw);
    } else {
      return std::make_shared<discrete_timed_robustness_condensing_json_monitor<time_t, value_t>>(
              pattern, kw);
    }
  }
};
